defmodule Homebudget.AccountsTest do
  ...
  describe "authenticate_by_username_and_password/2" do
    ...
    test "returns user with correct password", %{user: user} do
      assert {:ok, auth_user} = Accounts.authenticate_by_username_and_password(user.username, user.password)
    ...
    test "returns not found error with no matching user for email" do
      assert {:error, :not_found } = Accounts.authenticate_by_username_and_password("danny", "dummy")
    end
  end
end
